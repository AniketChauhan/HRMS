...
namespace HRMS.Controllers
{
    public class UnitMasterController : Controller
    {
            ...
            public ActionResult Index(string Data, string Search, int? page)
            {
                if (Data == "1")
                {
                    return View(db.UnitMaster.Where(x => x.UnitName.StartsWith(Search) || Search == null).ToList().ToPagedList(page ?? 1, 3));
                }
                else if (Data == "2")
                {
                    return View(db.UnitMaster.Where(x => x.Country.StartsWith(Search) || Search == null).ToList().ToPagedList(page ?? 1, 3));
                }
                else
                {
                    return View(db.UnitMaster.Where(x => x.City.StartsWith(Search) || Search == null).ToList().ToPagedList(page ?? 1, 3));
                }
            ...
            public ActionResult Create([Bind(Include = "UnitCode,UnitName,Address,Country,City,Pincode,Priority,PaySlip,IsActive,State")] UnitMaster unitMaster)
            {
                ...
                {
                    bool isValid = db.UnitMaster.Any(x => x.UnitName == unitMaster.UnitName || x.Priority == unitMaster.Priority);
                    if (!isValid)
                    {
                        unitMaster.IsActive = true;
                        db.UnitMaster.Add(unitMaster);
                        db.SaveChanges();
            ...
            public ActionResult Edit([Bind(Include = "UnitCode,UnitName,Address,Country,City,Pincode,Priority,PaySlip,IsActive,State")] UnitMaster unitMaster)
            {
                if (ModelState.IsValid)
                {
                    bool isValid = db.UnitMaster.Any(x => (x.UnitCode != unitMaster.UnitCode) && (x.UnitName == unitMaster.UnitName || x.Priority == unitMaster.Priority));
                    if (!isValid)
                    {
                    ...
                    {
                        if (db.UnitMaster.Any(x => (x.UnitCode != unitMaster.UnitCode) && (x.UnitName == unitMaster.UnitName)))
                        {
                ...
                long cid1 = db.Country.Where(x => x.CountryName == unitMaster.Country).Select(x => x.CountryID).SingleOrDefault();
                long sid1 = db.State.Where(x => x.StateName == unitMaster.State).Select(x => x.StateID).SingleOrDefault();
                ViewBag.Country = new SelectList(db.Country, "CountryName", "CountryName");
